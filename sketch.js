...
    { text: '第一單元作品', href: '#' },
    { text: '第一單元講義', href: '#' },
    { text: '期中作品筆記', href: '#' }, // 🌟 新增項目
    { text: '測驗系統', href: '#' },
    { text: '回到首頁', href: '#' } // 🌟 已加入關閉邏輯
  ];
...
    // 🌟 新增「期中作品筆記」邏輯
    if (it.text === '期中作品筆記') {
      const TARGET_URL = 'https://hackmd.io/@x-szOgytSz69ItDTKuKmRg/rJHoh6R1bl';
      a.elt.addEventListener('click', (ev) => {
        ev.preventDefault();
        openViewer(TARGET_URL);
      });
...
  
  // =================================================================
  // ✨ (2) 確保背景動態畫布與「林立靖」文字並存 (疊加在動畫之上)
  // =================================================================
  fill(255, 255, 255, 50); // 設定文字顏色為半透明白色
  noStroke(); 
  textAlign(CENTER, CENTER); 
  textSize(Math.floor(width / 12)); 
  // 繪製文字，位置 (width/2 + 160) 是為了避開左側選單
  text('林立靖', width / 2 + 160, height / 2); 
  // =================================================================
}
